...
export function ProviderLogo({
  ...
  const icon = providerId ? SIMPLE_ICONS[providerId] : undefined;

  if (providerId === "slack") {
    return <div className={className} aria-label="Slack logo"><SlackLogo /></div>;
  }
  if (providerId === "canva") {
    return <div className={`${className} provider-logo-wide`} aria-label="Canva logo"><CanvaLogo /></div>;
  }
  if (icon) {
    return (
      <div className={className} aria-label={`${icon.title} logo`}>
  ...
  return <div className={`${className} provider-logo-fallback`}>{fallback.slice(0, 2).toUpperCase()}</div>;
}

function SlackLogo() {
  return (
    <svg viewBox="0 0 127 127" role="img" aria-hidden="true">
